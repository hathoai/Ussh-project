package com.ussh.myapp.security.jwt;
...
class TokenAuthenticationIT {
    ...
    private void expectOk(String token) throws Exception {
        mvc.perform(MockMvcRequestBuilders.get("/api/authenticate").header(AUTHORIZATION, JwtAuthenticationTestUtils.BEARER + token)).andExpect(status().isOk());
    }
